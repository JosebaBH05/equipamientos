...
class Gestioa {
    ...
    // Función para insertar la ubicación
    public static function insert(int $ekipamendu_id, int $gela_id): bool {
        $db = DB::getConnection();
        // NOTA: Asegúrate de que idEkipamendu en inbentarioa sea un ID único o el comportamiento sea el deseado.
        $stmt = $db->prepare("INSERT INTO kokalekua (etiketa, idGela) SELECT i.etiketa, ? FROM inbentarioa i WHERE i.idEkipamendu = ?");
        return $stmt->execute([$gela_id, $ekipamendu_id]);
    }
}